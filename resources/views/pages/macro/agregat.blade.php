    ...
    <div class="card">
        ...
        <table class="table table-condensed" style="font-size: 12px">
            ...
            font-family: 'Times New Roman', Times, serif;">
                <td style="width: 150px;" colspan="2">{{ "Exercices" }}</td>
                ...
                @foreach($soussecteurs as $soussecteur)
                    @foreach($macroInSouSecteurs($dbs,$soussecteur->id) as $macro)
                        <tr style="text-align: center;">
                            ...
                            <td class="flex-md-grow-0">{{ html_entity_decode($macro->macro) }}</td>
                            @foreach($exercices as $exercice)
                                @if($loop->first && $input['naturep'] == 'paran')
                                    @php($bruteMPcdt = $FormatBrutMacro($getBruteMacro($dbs, $macro->id, $exercice->exercice-1)))
                                    @php($bruteSSPcdt = $FormatBrutMacro($getBruteSouSecteurAgreat($dbs,$soussecteur->id,$exercice->exercice-1)))
                                    @if($input['localite'] == 'uemoa')
                                        @php($bruteMUPcdt = $FormatBrutMacro($getBruteMacroUEMOA($macro->id, $exercice->exercice-1)))
                                        @php($bruteSSUPcdt =
                                        $FormatBrutMacro($getBruteSouSecteurAgreatUEMOA($soussecteur->id,$exercice->exercice-1)))
                                    @endif
                                @endif
                                <td> {{ $bruteM = $FormatBrutMacro($getBruteMacro($dbs, $macro->id, $exercice->exercice)) }}</td>
                                <th> {{ ($bruteMSS = $FormatBrutMacro($getBruteSouSecteurAgreat($dbs,$soussecteur->id,$exercice->exercice-1))) != 0 ?
                                round(($bruteM / $bruteMSS)*100,2) : '0.000'}}</th>
                                @if($request->get('localite') == 'uemoa')
                                    <th> {{ ($bruteMU = $FormatBrutMacro($getBruteMacroUEMOA($macro->id, $exercice->exercice))) != 0 ?
                                    round(($bruteM / $bruteU)*100,2) : '0.000' }} </th>
...
{{--                                        style="color: {{ ($diff = round(($bruteM - $bruteMPcdt)*100,2)) != 0 ? ($diff > 0 ? 'green' : 'red') : ''}}"--}}
                                    <th> {{ $diff = round(($bruteM - $bruteMPcdt)*100,2) }}</th>
