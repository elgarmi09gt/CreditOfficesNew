...
@php($RATIO_V = collect())
@php($RATIOS = collect())
@foreach($Countries as $countrie) {{-- For Any Country Calculate ccc for either Year --}}
    @foreach($exercices as $exercice)
        @php($RATIO[0][0] = $countrie->idPays)
        @php($RATIO[0][1] = $exercice->exercice)
        @php($RATIO[0][2] = $cccPays($exercice->exercice,$countrie->bdPays))
        @if($input['naturep']=='paran') {{-- Evolution if By Year--}}
            @php($RATIO[0][3] = $cccPays($exercice->exercice,$countrie->bdPays) - $cccPays($exercice->exercice-1,$countrie->bdPays))
        @endif
        @php($RATIOS = $RATIOS->concat($RATIO))
    @endforeach
   @php($RV[0][0] = $countrie->idPays)
   @php($RV[0][1] = $cccPays($exercice2,$countrie->bdPays) - $cccPays($exercice1,$countrie->bdPays))
    @php($RATIO_V = $RATIO_V->concat($RV))
@endforeach
@foreach($exercices as $exercice) {{-- Calculate ccc For UEMOA for etheir YEAR --}}
    @php ($RU[0][0] = ($exercice->exercice))
    @php ($RU[0][1] = $cccUEMOA($exercice->exercice))
    @if($input['naturep'] == 'paran') {{-- UEMOA Evolution If By YEAR --}}
        @php($RU[0][2] = $cccUEMOA($exercice->exercice) - $cccUEMOA($exercice->exercice-1))
    @endif
    @php($RATIO_U = $RATIO_U->concat($RU))
@endforeach
@if($input['naturep'] != 'paran')
    @php($RATIO_VU [] = ($cccUEMOA($exercice2) - $cccUEMOA($exercice1)))
@endif
        ...
        <tr>
            ...
            @if($input['naturep'] != "paran")
                <th colspan="4" style="text-align: center;background-color: #8ec5fc;">
                    <div class="col">{{ 'Pays : '. ($cccPays($exercice2,$countrie->bdPays) - $cccPays($exercice1,$countrie->bdPays))}}</div>
                    <div class="col">{{ 'UEMOA : '. ($cccUEMOA($exercice2) - $cccUEMOA($exercice1))}}</div>
                </th>
            @endif
        </tr>
        <tr>
            ...
            <tr>
                ...
                @foreach($exercices as $exercice)
                    @if($loop->first && $input['naturep'] == "paran")
                        @php($ep = $ccc($entreprise->idEntreprise,$exercice->exercice - 1,$countrie->bdPays))
                    @endif
                    @foreach($RATIOS as $r)
                        @continue($r[0] != $countrie->idPays || $r[1] != $exercice->exercice)
                        @foreach($RATIO_U as $ru)
                            @continue($ru[0] != $r[1])
                            <th style="background-color: #e2ebf0">{{ $e = $ccc($entreprise->idEntreprise,$exercice->exercice,$countrie->bdPays) }}</th>
                ...
                @if($input['naturep'] != "paran")
                    @foreach($RATIO_V as $rv)
                        ...
                        @php($diff = ($ccc($entreprise->idEntreprise,$exercice2,$countrie->bdPays) - ($ep = $ccc($entreprise->idEntreprise,$exercice1,$countrie->bdPays))))
