...
@php($RATIO_V = collect())
@php($RATIOS = collect())
@foreach($Countries as $countrie) {{-- For Any Country Calculate rif for either Year --}}
    @foreach($exercices as $exercice)
        @php($RATIO[0][0] = $countrie->idPays)
        @php($RATIO[0][1] = $exercice->exercice)
        @php($ps = ($RATIO[0][2] = $rifPays($exercice->exercice,$countrie->bdPays)))
        @if($input['naturep'] =='paran') {{-- Evolution if By Year--}}
            @if ($loop->first)
                @php($pp = $rifPays($exercice->exercice-1,$countrie->bdPays))
            @endif
            @php($RATIO[0][3] = $ps - $pp)
        @endif
    ...
    @if($input['naturep'] != 'paran')
        ...
        @php($RV[0][1] = $rifPays($exercice2,$countrie->bdPays) - $rifPays($exercice1,$countrie->bdPays))
        @php($RATIO_V = $RATIO_V->concat($RV))
    @endif
@endforeach
@foreach($exercices as $exercice) {{-- Calculate rif For UEMOA for etheir YEAR --}}
    @php ($RU[0][0] = ($exercice->exercice))
    @php ($ps = ($RU[0][1] = $rifUEMOA($exercice->exercice)))
    @if($input['naturep'] == 'paran') {{-- UEMOA Evolution If By YEAR --}}
        @if($loop->first)
            @php($pp = $rifUEMOA($exercice->exercice-1))
            @php($RU[0][2] = $ps - $pp)
        @endif
    @endif
...
@if($input['naturep'] != 'paran')
    @php($RATIO_VU [] = ($rifUEMOA($exercice2) - $rifUEMOA($exercice1)))
@endif
        ...
        <tr>
            ...
            @if($input['naturep'] != "paran")
                <th colspan="4" style="text-align: center;background-color: #8ec5fc;">
                    <div class="col">{{ 'Pays : '. ($rifPays($exercice2,$countrie->bdPays) - $rifPays($exercice1,$countrie->bdPays))}}</div>
                    <div class="col">{{ 'UEMOA : '. ($rifUEMOA($exercice2) - $rifUEMOA($exercice1))}}</div>
                </th>
            @endif
        </tr>
        <tr>
            ...
            <tr>
                ...
                @foreach($exercices as $exercice)
                    @if($loop->first && $input['naturep'] == "paran")
                        @php($ep = $rif($entreprise->idEntreprise,$exercice->exercice - 1,$countrie->bdPays))
                    @endif
                    @foreach($RATIOS as $r)
                        @continue($r[0] != $countrie->idPays || $r[1] != $exercice->exercice)
                        @foreach($RATIO_U as $ru)
                            @continue($ru[0] != $r[1])
                            <th style="background-color: #e2ebf0">{{ $e = $rif($entreprise->idEntreprise,$exercice->exercice,$countrie->bdPays) }}</th>
                ...
                @if($input['naturep'] != "paran")
                    @foreach($RATIO_V as $rv)
                        ...
                        @php($diff = ($rif($entreprise->idEntreprise,$exercice2,$countrie->bdPays) - ($ep = $rif($entreprise->idEntreprise,$exercice1,$countrie->bdPays))))
