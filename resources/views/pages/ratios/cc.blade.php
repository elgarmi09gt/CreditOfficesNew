...
@php($RATIO_V = collect())
@php($RATIOS = collect())
@foreach($Countries as $countrie) {{-- For Any Country Calculate cc for either Year --}}
        ...
        <tr>
            ...
            @if($input['naturep'] != "paran")
                <th colspan="4" style="text-align: center;background-color: #8ec5fc;">
                    <div class="col">{{ 'Pays : '. ($ccPays($exercice2,$countrie->bdPays) - $ccPays($exercice1,$countrie->bdPays))}}</div>
                    <div class="col">{{ 'UEMOA : '. ($ccUEMOA($exercice2) - $ccUEMOA($exercice1))}}</div>
                </th>
            @endif
        </tr>
        <tr>
            ...
            <tr>
                ...
                @foreach($exercices as $exercice)
                    @if($loop->first && $input['naturep'] == "paran")
                        @php($ep = $cc($entreprise->idEntreprise,$exercice->exercice - 1,$countrie->bdPays))
                    @endif
                    @foreach($RATIOS as $r)
                        @continue($r[0] != $countrie->idPays || $r[1] != $exercice->exercice)
                        @foreach($RATIO_U as $ru)
                            @continue($ru[0] != $r[1])
                            <th style="background-color: #e2ebf0">{{ $e = $cc($entreprise->idEntreprise,$exercice->exercice,$countrie->bdPays) }}</th>
                ...
                @if($input['naturep'] != "paran")
                    @foreach($RATIO_V as $rv)
                        ...
                        @php($diff = ($cc($entreprise->idEntreprise,$exercice2,$countrie->bdPays) - ($ep = $cc($entreprise->idEntreprise,$exercice1,$countrie->bdPays))))
