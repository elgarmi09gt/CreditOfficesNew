...
class CreateSecteurMacrosTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        for ($i = 0; $i < count($BD); $i++) {
            Schema::connection($BD[$i])->create('secteur_macros', function (Blueprint $table) {
    ...
    public function down()
    {
        $BD = array('bic_bd_test', 'bic_beninbd', 'bic_bissaubd', 'bic_burkinabd', 'bic_coteivoirbd', 'bic_malibd',
            ...
            for ($i = 0; $i < count($BD); $i++) {
                Schema::connection($BD[$i])->dropIfExists('secteur_macros');
            }
    }
}
